...
public final class SubgroupGenerator {
    ...
    /**
     * Compute the center Z(G) = {g ∈ G : gx = xg for all x ∈ G}.
     * <p>
     * The center consists of all elements that commute with every element in the
     * group. It is always a normal subgroup.
     * </p>
     * <p>
     * Properties:
     * </p>
     * <ul>
     * <li>Z(G) is always normal (in fact, characteristic)</li>
     * <li>For abelian groups, Z(G) = G</li>
     * <li>For non-abelian groups, Z(G) is a proper subgroup</li>
     * <li>|Z(G)| = 1 if and only if G has trivial center</li>
     * </ul>
     * 
     * @param parent the parent group
     * @return the center subgroup
     */
    public static <E extends Element> Subgroup<E> center(Group<E> parent) {
        Objects.requireNonNull(parent, PARENT_NULL_MSG);

        Set<E> centerElements = new HashSet<>();
        Set<E> elements = parent.elements();

        for (E g : elements) {
            boolean inCenter = true;
            for (E x : elements) {
                E gx = parent.operate(g, x);
                E xg = parent.operate(x, g);
                if (!gx.equals(xg)) {
                    inCenter = false;
                    break;
                }
            ...
            if (inCenter) {
                centerElements.add(g);
            }
        }

        return new Subgroup<>(parent, centerElements);
    }

    /**
     * Compute the commutator subgroup [G,G] (also called the derived subgroup).
     * <p>
     * The commutator subgroup is the subgroup generated by all commutators
     * [g,h] = g⁻¹h⁻¹gh for g,h ∈ G. It is always normal in G.
     * </p>
     * <p>
     * Properties:
     * </p>
     * <ul>
     * <li>[G,G] is always normal</li>
     * <li>G/[G,G] is abelian (the largest abelian quotient)</li>
     * <li>[G,G] = {e} if and only if G is abelian</li>
     * <li>For S_n, [S_n,S_n] = A_n (alternating group)</li>
     * </ul>
     * 
     * @param parent the parent group
    ...
    public static <E extends Element> Subgroup<E> commutatorSubgroup(Group<E> parent) {
        ...
        return generate(parent, commutators);
    }
}
